...
def extract_uniformat_number(elem):
    """Extrait le code Uniformat depuis les associations de classification"""
...
def extract_uniformat_description(elem):
    """Extrait la VRAIE description Uniformat depuis les associations de classification"""
    if hasattr(elem, "HasAssociations"):
        for assoc in elem.HasAssociations:
            if assoc.is_a("IfcRelAssociatesClassification"):
                ref = assoc.RelatingClassification
                # CORRECTION : Récupérer la vraie description depuis le champ Description
                # Dans IFCCLASSIFICATIONREFERENCE, la description est le 5ème paramètre
                if hasattr(ref, "Description") and ref.Description:
                    desc = str(ref.Description).strip()
                    if desc and desc != "":  # Éviter les descriptions vides
                        return desc
                # Fallback : si pas de Description, essayer Name mais éviter "Uniformat"
                elif hasattr(ref, "Name") and ref.Name:
                    name = str(ref.Name).strip()
                    if name != "Uniformat" and name != "":  # Éviter la valeur non-informative
                        return name
    return None

def extract_material(elem):
    """Extrait le matériau depuis les associations de matériau ou les PropertySets"""
    # Méthode 1: Associations de matériau (standard IFC)
    if hasattr(elem, "HasAssociations"):
        for assoc in elem.HasAssociations:
            if assoc.is_a("IfcRelAssociatesMaterial"):
                ...
                # Si c'est un IfcMaterialLayerSetUsage ou IfcMaterialLayerSet
                    ...
                    if hasattr(layer_set, "MaterialLayers") and layer_set.MaterialLayers:
                        ...
                        for layer in layer_set.MaterialLayers:
                            if hasattr(layer, "Material") and layer.Material and hasattr(layer.Material, "Name"):
                                materials.append(str(layer.Material.Name))
                        if materials:
                            return " / ".join(materials)
    
    # Méthode 2: PropertySets (fallback)
                ...
                if hasattr(prop_set, "HasProperties"):
                    for prop in prop_set.HasProperties:
                        if hasattr(prop, "Name") and prop.Name:
                            ...
                            if any(keyword in prop_name for keyword in ['material', 'matériau', 'matiere']):
                                if hasattr(prop, "NominalValue") and prop.NominalValue:
                                    return str(prop.NominalValue.wrappedValue).strip()
    
    return None

def extract_uniformat_props(elem):
    """Retourne (uniformat_code, uniformat_desc) extraits UNIQUEMENT des vraies références Uniformat IFC."""
    uniformat_code = None
    uniformat_desc = None
    
    # UNIQUEMENT les associations de classification Uniformat officielles
    # Ignorer complètement les PropertySets qui contiennent des classifications système
    if hasattr(elem, "HasAssociations"):
        for assoc in elem.HasAssociations:
            if assoc.is_a("IfcRelAssociatesClassification"):
                ref = assoc.RelatingClassification
                
                # Vérifier que c'est une vraie référence Uniformat
                if hasattr(ref, "Location") and ref.Location:
                    location = str(ref.Location).lower()
                    # Accepter uniquement les références avec l'URL Uniformat officielle
                    if "uniformat" in location and "csiresources.org" in location:
                        # Extraire le code Uniformat
                        if hasattr(ref, "ItemReference") and ref.ItemReference:
                            uniformat_code = str(ref.ItemReference).strip()
                        
                        # Extraire la description Uniformat
                        if hasattr(ref, "Description") and ref.Description:
                            desc = str(ref.Description).strip()
                            if desc and desc != "" and desc != "Uniformat":
                                uniformat_desc = desc
                        
                        # Si on a trouvé une vraie référence Uniformat, on s'arrête
                        if uniformat_code or uniformat_desc:
                            break
    
    # NE PAS utiliser les PropertySets comme fallback car ils contiennent
    # des classifications système (Sanitaire, Soufflage, etc.) qui ne sont pas Uniformat
    
    return uniformat_code, uniformat_desc
def extract_ifc_class(elem):
    """Extrait la classe IFC de l'élément"""
    return elem.is_a() if elem else None
...
@app.route('/fix-uniformat-data', methods=['POST'])
def fix_uniformat_data():
    """Corrige les données Uniformat inversées dans l'ontologie"""
    try:
        # Récupérer tous les éléments avec des données Uniformat
        sparql_query = """
        PREFIX wlc: <http://www.semanticweb.org/adamy/ontologies/2025/WLCONTO#>
        SELECT ?elem ?uniformat ?uniformatDesc
        WHERE {
          ?elem wlc:hasUniformatCode ?uniformat .
          OPTIONAL { ?elem wlc:hasUniformatDescription ?uniformatDesc . }
        }
        """
        ...
        for row in results:
            elem_uri = row['elem']
            current_code = row.get('uniformat', '')
            current_desc = row.get('uniformatDesc', '')
            
            # Détecter si les données sont inversées
            should_swap = False
            
            # Si le "code" est long et descriptif (probablement une description)
            if current_code and len(current_code) > 15 and not any(char.isdigit() or char == '.' for char in current_code):
                should_swap = True
            
            # Si on a les deux et ils semblent inversés
            elif current_code and current_desc:
                if (len(current_code) > len(current_desc) and 
                    not any(char.isdigit() or char == '.' for char in current_code) and
                    any(char.isdigit() or char == '.' for char in current_desc)):
                    should_swap = True
            ...
            if should_swap:
                ...
                PREFIX wlc: <http://www.semanticweb.org/adamy/ontologies/2025/WLCONTO#>
                DELETE {{
                    <{elem_uri}> wlc:hasUniformatCode ?oldCode .
                    <{elem_uri}> wlc:hasUniformatDescription ?oldDesc .
                }}
                WHERE {{
                    OPTIONAL {{ <{elem_uri}> wlc:hasUniformatCode ?oldCode . }}
                    OPTIONAL {{ <{elem_uri}> wlc:hasUniformatDescription ?oldDesc . }}
                }}
                """
                update_graphdb(delete_query)
                
                # Insérer les nouvelles valeurs corrigées
                if current_desc:  # L'ancien desc devient le nouveau code
                    insert_uniformat_code(elem_uri, current_desc)
                if current_code:  # L'ancien code devient la nouvelle desc
                    insert_uniformat_description(elem_uri, current_code)
                
                corrections_made += 1
            
            # Cas spécial : si on n'a qu'un "code" qui ressemble à une description
            elif current_code and not current_desc and len(current_code) > 15:
                # Déplacer vers description et vider le code
                delete_query = f"""
                PREFIX wlc: <http://www.semanticweb.org/adamy/ontologies/2025/WLCONTO#>
                DELETE {{
                    <{elem_uri}> wlc:hasUniformatCode ?oldCode .
        ...
        return jsonify({
            'success': True,
            'message': f'Correction terminée. {corrections_made} éléments corrigés.',
